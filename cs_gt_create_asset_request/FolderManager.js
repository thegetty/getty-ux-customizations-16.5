(function(exports) {
	/**
	 * The FolderManager object interfaces with the REST API services using
	 * standard CRUD methodology, and serves as the caching mechanism to manage
	 * and provide access to folder objects.
	 * 
	...
	/*
	 * otui.ready(function() { FolderManager.getChildCounts =
	 * (otui.SystemSettingsManager.getSystemSettingValue("UX", "GENERAL",
	 * "DISPLAY_FOLDER_CHILD_COUNTS") != "false"); });
	 */

	FolderManager.read = function read(data, callback) {
		var nodeID = data.nodeID;
		var isRoot = (nodeID == "ROOT_FOLDERS");
		var folderLimit = otui.SystemSettingsManager.getSystemSettingValue(
				"UX", "GENERAL", "NUM_OF_FOLDERS_TO_LOAD");
		...
		// var data =
		// {"load_metadata":"true","metadata_fields_to_return":["ARTESIA.FIELD.CONTAINER
		// TYPE NAME","ARTESIA.FIELD.IMPORTED BY","ARTESIA.FIELD.MODEL"]};
		// var data = {};

		// ART-24462 Due to performance bottlenecks, we have disabled the
		// "asset" child counting (This does not affect the results list). There
		// is also the option to completely turn off the counts.
		// Once these bottlenecks are resolved, we can remove this area.
		/*
		 * if (isRoot) params.push('generate_child_count=' +
		 * !!(FolderManager.getChildCounts)); else data['child_count_load_type'] =
		 * FolderManager.getChildCounts ? "folders" : "none";
		 * 
		 * params.push("data_load_request=" +
		 * encodeURIComponent(JSON.stringify({"data_load_request": data})));
		 */

		var serviceUrl = (isRoot ? otui.service + "/folders/rootfolders/"
				: otui.service + "/folders/" + nodeID + "/folders")
				+ "?" + params.join("&");
				...
				.get(
						...
						function(response, status, success) {
							...
							if (!success) {
								...
								var myFoldersID = UserDetailsManager
										.getUserDetails().user_id
										+ "N";
								var myOrphansID = UserDetailsManager
										.getUserDetails().user_id
										+ "Y";

								for (var i = 0; i < jResponse.length; ++i) {
									var node = jResponse[i];
									// If hideOrphans is true and we receive My
									// Orphans/ Public Orphans from server then
									// skip it.
									if (data.hideOrphans
											&& (node.asset_id === myOrphansID || node.asset_id === "ARTESIA.PUBLIC.TREEY"))
										continue;
									if ((node.asset_id != myFoldersID && node.asset_id != "ARTESIA.PUBLIC.TREEY")
											|| ((node.asset_id == myFoldersID && otui.UserFETManager
													.isTokenAvailable("FOLDER.CREATE")) || (node.asset_id == "ARTESIA.PUBLIC.TREEY" && otui.UserFETManager
													.isTokenAvailable("FOLDER.PUBLIC_ORPHANS")))) {
										...
										// Root folders have orphans folders
										// which don't appear with the counts,
										// so make sure they have at least one.
										// Also if we don't have child counting
										// switched on, then hardcode it to 1.
										if (isRoot
												|| !FolderManager.getChildCounts)
									...
									if (node.asset_id == "ARTESIA.PUBLIC.TREEY")
										node.name = otui.tr("Public Orphans");
									if (node.asset_id == "ARTESIA.PUBLIC.TREEN")
										node.name = otui.tr("Public Folders");
									if (node.asset_id == myOrphansID)
										node.name = otui.tr("My Orphans");
									if (node.asset_id == myFoldersID)
										node.name = otui.tr("My Folders");
								}
	...
	// var serviceUrl = otui.service +
	// "/folders/rootfolders/?load_type=system&limit=0";
	...
	// for (var i = 0; i < jResponse.length; ++i)
	// {
	// serviceUrlArray[i] = otui.service + "/folders/" + jResponse[i].asset_id +
	// "/folders?load_type=system&limit=0";
	...
	// FolderManager.getFoldersNames(text, data.folders_resource.folder_list,
	// "Folders name");
	// resolve(text);
	// })
	// .catch(function(err)
	...
	FolderManager.setFolderThumbnail = function setFolderThumbnail(folderID,
			blob, renditionType, callback) {
		...
		otui.post(serviceUrl, data, otui.contentTypes.multipart, function(
				serverData, status, success) {
			callback(serverData, status, success);
		});
	};

	FolderManager.removeFolderThumbnail = function removeFolderThumbnail(
			folderID, callback) {
		var serviceUrl = otui.service + "/folders/" + folderID + "/contents";
	...
	FolderManager.createFolder = function createFolder(props) {
		...
		if (props.securityPolicies) {
			...
			for ( var securityPolicy in props.securityPolicies) {
				securityPolicies.push({
					"id" : props.securityPolicies[securityPolicy]
				});
			}
		...
		otui
				.post(
						...
						function(response, status, success) {
							var message = otui
									.tr("Folder created successfully.");
							var stayOpen = false;
							var status = "ok";
							...
							if (!success) {
								...
								if (props.type == 'GTRUST.FOLDER.ASSET REQUEST') {
									RequestFolderManager
											.setRecentAssetRequest({
												'name' : name,
												'asset_id' : asset_id
											})
								}
								FolderManager
										.clearCachedFolderDataForID(props.folderID);
								if (!FolderManager.getChildCounts)
									response.folder_resource.folder.container_child_count = 1;

								var folderSidebarView = otui.main
										.getChildView(FolderSidebarView);
								if (folderSidebarView) {
									var folderView = folderSidebarView
											.getChildView(FolderView);
									if (folderView)
										folderView
												.addNewNodes(
														[ response.folder_resource.folder ],
														props.folderID, true);
								}
							}
	...
	FolderManager.addAssetsToFolder = function addAssetsToFolder(event) {
		if (FolderManager.selectedFolderID
				&& FolderManager.selectedFolderID.length > 0) {
			var view = otui.Views.containing($(".ot-results")[0]);
			// For relationship and version tab in asset inspector view,
			// pagination will not be there
			// so just use for folder asset action element to retrive the
			// current view
			if (!view) {
			...
			if (moveAssets.selected) {
				...
				if (view
						&& (view instanceof FolderResultsView || view instanceof InspectorView)) {
			...
			otui.services.folderChildren
					.update(
							{
								'nodeID' : FolderManager.selectedFolderID,
								'action' : action,
								'fromID' : fromID,
							...
							function(response, success) {
								if (success
										&& response.folder_operation_resource) {
									...
									if (numSucceeded) {
										if (action == 'add')
											message
													.push(otui
															.trn(
																	'{0} asset copied successfully.',
																	'{0} assets copied successfully.',
																	numSucceeded,
																	numSucceeded));
										else
											message
													.push(otui
															.trn(
																	'{0} asset moved successfully.',
																	'{0} assets moved successfully.',
																	numSucceeded,
																	numSucceeded));

										FolderManager
												.clearCachedFolderDataForID(FolderManager.selectedFolderID);
									}
									if (numFailed) {
										...
										if (numSucceeded == 0
											...
											if (action == 'add')
												message
														.push(otui
																.tr('No selected assets could be copied to your folder.'));
											else
												message
														.push(otui
																.tr('No selected assets could be moved to your folder.'));
										} else if (existingCount < numFailed) {
											message.push(otui.trn(
													'{0} asset failed.',
													'{0} assets failed.',
													numFailed - existingCount,
													numFailed - existingCount));
										}
										...
										if (existingCount) {
											if (action == 'add')
												message
														.push(otui
																.trn(
																		'{0} asset already exists in the selected folder.',
																		'{0} assets already exist in the selected folder.',
																		existingCount,
																		existingCount));
											else
												message
														.push(otui
																.trn(
																		'{0} asset removed from source folder, but already existed in the selected destination folder.',
																		'{0} assets removed from source folder, but already existed in the selected destination folder.',
																		existingCount,
																		existingCount));
										}
									}
									...
									if (!numSucceeded && !numFailed) {
										...
										if (action == 'add')
											message
													.push(otui
															.tr('No assets could be copied.'));
										else
											message
													.push(otui
															.tr('No assets could be moved.'));
									}
									...
									otui.NotificationManager
											.showNotification({
												'message' : otui
														.tr('All assets failed to be copied/moved.'),
												'stayOpen' : true,
												'status' : 'error'
											});
								}

								if ((numSucceeded || existingCount)
										&& action == 'move') {
	...
	FolderManager.rememberFolder = function rememberFolder(req) {
		if (req.params.nodeID !== RecentArtifactsManager.RECENT_FOLDERS) {
			...
			if (appliedFacets.length == 0
					&& (req.params.searchConfigID == undefined || req.params.searchConfigID == "none"))
				_lastFolderParams = {
					'nodeID' : req.params.nodeID,
					'breadcrumb' : req.params.breadcrumb,
					'pageProperties' : req.params.pageProperties,
					'appliedFacets' : req.params.appliedFacets,
					'filterTerm' : "",
					'searchConfigID' : "none",
					'searchScopeID' : "none"
				};
				...
				_lastFolderParams.pageProperties = PageManager
						.createPageProperties(
								0,
								req.params.pageProperties.assetsPerPage,
								PageManager.userSortPrefsMap["FolderResultsView"].sortOrder,
								PageManager.userSortPrefsMap["FolderResultsView"].sortField);
			}
		} else {
			_lastRecentFoldersParams = {
				'nodeID' : req.params.nodeID,
				'pageProperties' : req.params.pageProperties
			};
	...
	FolderManager.rememberFolderSearch = function rememberFolderSearch(req) {
		...
		if (appliedFacets.length > 1
				|| (req.params.searchConfigID && req.params.searchConfigID != "none"))
			_lastFolderSearchParams = {
				'nodeID' : req.params.nodeID,
				'breadcrumb' : req.params.breadcrumb,
				'pageProperties' : req.params.pageProperties,
				'appliedFacets' : req.params.appliedFacets,
				'filterTerm' : req.params.filterTerm,
				'searchConfigID' : req.params.searchConfigID,
				'searchScopeID' : req.params.searchScopeID,
				'currentAccordion' : req.params.currentAccordion,
				'advancedSearch' : req.params.advancedSearch,
				'savedSearchID' : req.params.savedSearchID,
				'metadataLocale' : req.params.metadataLocale
			};
		else if (!_lastFolderSearchParams
				|| !_lastFolderSearchParams.pageProperties) {
			_lastFolderSearchParams = {
				'nodeID' : req.params.nodeID,
				'breadcrumb' : req.params.breadcrumb,
				'appliedFacets' : req.params.appliedFacets,
			...
			_lastFolderSearchParams.pageProperties = PageManager
					.createPageProperties(
							0,
							req.params.pageProperties.assetsPerPage,
							PageManager.userSortPrefsMap["SearchView"].sortOrder,
							PageManager.userSortPrefsMap["SearchView"].sortField);
		}
	...
	FolderManager.performAdvancedSearch = function(folderID, keyword,
			breadcrumb, searchConfigID, searchConfigName, searchConditionList,
			sortField, sortOrder, recentSearch, metadataLocale, searchScopeID,
			searchScopeName, appliedFacets) {
		var searchFunction = FolderResultsView.route.use("open", true);

		if (searchFunction) {
			var pageProperties = PageManager.createPageProperties(0,
					PageManager.assetsPerPage, sortOrder, sortField);
			...
			SearchManager.clearSearchForKeyword(folderID + "_" + searchConfigID
					+ "_" + keyword);

			if (!_lastFolderSearchParams
				...
				_lastFolderSearchParams.appliedFacets = appliedFacets
						|| JSON.stringify({
							'facets' : []
						});
				_lastFolderSearchParams.advancedSearch = JSON.stringify({
					'search_condition_list' : searchConditionList
				});
	...
	FolderManager.performKeywordSearch = function(term, recentSearch) {
		...
		if (filterTerm && filterTerm.length > 0) {
			...
			if (searchFunction) {
				...
				var breadcrumb = recentSearch ? recentSearch.breadcrumb
						: currentView.properties.breadcrumb;
				...
				if (!recentSearch) {
					...
					if (recentSearch.folderId)
						folderID = recentSearch.folderId;
				}

				SearchManager.clearSearchForKeyword(folderID + "_"
						+ searchConfigID + "_" + filterTerm);
				...
				var folderResultsView = otui.main
						.getChildView(FolderResultsView);
				...
				if (!_lastFolderSearchParams
					...
					searchFunction('open', {
						...
						'breadcrumb' : breadcrumb,
						'pageProperties' : pageProperties,
						'savedSearchID' : savedSearchID[1] || 'none',
						'advancedSearch' : JSON.stringify({
							"search_condition_list" : undefined
						}),
						'appliedFacets' : JSON.stringify({
							'facets' : []
						}),
					...
					_lastFolderSearchParams.nodeID = folderID;
					_lastFolderSearchParams.breadcrumb = breadcrumb;
					_lastFolderSearchParams.appliedFacets = JSON.stringify({
						'facets' : []
					});
					_lastFolderSearchParams.advancedSearch = JSON.stringify({
						"search_condition_list" : undefined
					});
					_lastFolderSearchParams.savedSearchID = savedSearchID[1]
							|| 'none';
					searchFunction('open', _lastFolderSearchParams);
				}
			}
	...
	FolderManager.resetRememberedFolderSearch = function resetRememberedFolder() {
		if (_lastFolderSearchParams && _lastFolderSearchParams.pageProperties) {
			// Copy the parameters so they don't get altered in the properties.
			_lastFolderSearchParams = jQuery.extend(true, {},
					_lastFolderSearchParams);
	...
	FolderManager.route = function route() {
		if (!_lastFolderParams && !_lastSelectedRecentFolders) {
			...
			if (_lastSelectedRecentFolders) {
				var recentFoldersResultsViewFunction = RecentFoldersResultsView.route
						.use("open");
				recentFoldersResultsViewFunction('open',
						_lastRecentFoldersParams);
			} else {
				var foldersResultsViewFunction = FolderResultsView.route
						.use("open");
	...
	function readFolderData(nodeID, fullPath, callback) {
		...
		if (fullPath) {
			dataLoadRequest = {
				'child_count_load_type' : 'both',
				'load_path_with_children' : true
			};
		} else {
			dataLoadRequest = {
				'child_count_load_type' : 'both',
		...
		var serviceUrl = "/folders/" + nodeID
				+ '?load_type=custom&data_load_request='
				...
				.get(
						...
						function(response, status, success) {
							if (!success)
								...
								FolderManager.updateFolderData(nodeID,
										folderData);
	...
	function doFolderChildrenRead(data, callback, view) {
		...
		var after = function after(folderData) {
			...
			var preferenceName = PageManager
					.getFieldPreferenceName(view.properties.templateName);
			...
			AssetManager.getAssets(data.nodeID, data.pageProperties,
					assetsPerPage, preferenceName, extraFields, callback,
					data.isWidget);
		};

		var folderSidebarView = typeof FolderSidebarView === 'undefined' ? undefined
				: otui.main.getChildView(FolderSidebarView);
		var fullPath = false;
		// if(cachedData && (!cachedData.path_list[0].parents ||
		// cachedData.path_list[0].parents &&
		// !cachedData.path_list[0].parents.length) && folderSidebarView &&
		// !folderSidebarView.getChildView(FolderView).model.find(nodeID))
		// {
		...
		// }
		if (folderSidebarView
				&& !folderSidebarView.getChildView(FolderView).model
						.find(nodeID)) {
			cachedData = undefined;
			FolderManager.clearCachedFolderDataForID(nodeID);
			fullPath = true;
		...
		if (cachedData && nodeID !== RecentArtifactsManager.RECENT_FOLDERS) {
			...
			if (nodeID === RecentArtifactsManager.RECENT_FOLDERS) {
				otui.services.recentfolderswithpathlist.read({
					'nodeID' : nodeID
				}, callback, this);
			} else {
	...
	function doFolderChildrenSearch(data, callback, view, pageID, appliedFacets) {
		...
		SearchManager.getFolderAssetsByKeyword(filterTerm,
				data.pageProperties.page, data.pageProperties.assetsPerPage,
				preferenceName, extraFields, data, function(results,
						totalItems, success) {
	...
	function folderChildrenRead(data, callback, view) {
		...
		if (!otui.UserFETManager.isTokenAvailable("FOLDERS.VIEW")) {
			view.storedProperty("no-results-msg",
		...
		if ((hasFacets || hasSearchTerms)
				&& (("searchConfigID" in data) || data.searchID))
		...
		var sendResults = function sendResults(response, success) {
			...
			var numTotalChildren = data.pageProperties.assetsPerPage
					* data.pageProperties.page + response.length;
			var lastPage = view.properties.ot_start_page == view.properties.ot_end_page;

			if (lastPage
					&& numTotalChildren != PageManager.numTotalChildren[pageID]) {
				PageManager.numTotalChildren[pageID] = numTotalChildren;
				PageManager.updatePaging(view, pageID);
			}

			if (lastPage
					&& response.length == data.pageProperties.assetsPerPage) {
				FolderManager.clearCachedFolderDataForID(nodeID);
			...
			if (!view.properties.folderData) {
				FolderManager.getFolderData(nodeID, function(folderData) {
					...
					// var cachedData =
					// FolderManager.getCachedFolderData(nodeID);
					//					
					// if(!cachedData)
		...
		if (needsSearch)
			doFolderChildrenSearch(data, sendResults, view, pageID,
					appliedFacets);
		else
	...
	otui.registerService('folderChildren', {
		...
		'update' : function(data, callback) {
			...
			otui.put(otui.service + serviceUrl, params,
					otui.contentTypes.formData, function(response, status,
							success) {
						callback(response, success);
					});
		}
	...
	/**
	 * This function returns whether the folder has permissions to upload assets
	 * or not
	 * 
	 * @param nodeID
	 * @returns folderPermissions
	 */
	FolderManager.folderCanContainAssets = function folderCanContainAssets(
			folder) {
	...
	/**
	 * This function returns whether the folder has permission to edit content
	 * or not
	 * 
	 * @param nodeID
	...
	/**
	 * This function returns whether the folder is marked for Deletion. Assets
	 * should not be uploaded in this type of folders.
	 * 
	 * @param response
	 * @returns folderPermissions
	 */
	FolderManager.isFolderMarkedForDeletion = function isFolderMarkedForDeletion(
			folder) {
		var folderPermissions = false;
		if (folder !== undefined) {
			if (folder.asset_state === "LOCKED"
					&& folder.content_state === "DELETED") {
	...
	/**
	 * This function returns whether the folder is container children
	 * restricted. Sub folders should not be created in this type of folders.
	 * 
	 * @param response
	 * @returns containerChildrenRestricted
	 */

	FolderManager.isContainerChildrenRestricted = function isContainerChildrenRestricted(
			folder) {
		var containerChildrenRestricted = false;
		var folderTypeObj = FolderTypesManager
				.getFolderTypeById(folder.container_type_id);
		if (folderTypeObj && folderTypeObj.container_children_restricted
				&& !folderTypeObj.child_types[0]) {
	...
	FolderManager.disableUploadForNode = function disableUploadForNode(nodeID) {
		...
		if (nodeID == "ARTESIA.PUBLIC.TREEY" || nodeID == userID + "Y"
				...
				|| nodeID === RecentArtifactsManager.RECENT_FOLDERS
				|| otui.UploadUtils.inTransfer()) {
			return true;
		}
	...
	FolderManager.isRootFolder = function(id) {
		return (FolderManager.isPublicFolders(id) || FolderManager
				.isMyFolders(id));
	};

	FolderManager.isOrphanFolder = function(id) {
		return (FolderManager.isPublicOrphan(id) || FolderManager
				.isPrivateOrphan(id));
	};
	...
	FolderManager.folderSupportsFolderCreation = function(data) {
		...
		var isChildrenRestricted = FolderManager
				.isContainerChildrenRestricted(data);
		return otui.UserFETManager.isTokenAvailable("FOLDER.CREATE")
				&& !FolderManager.isPrivateOrphan(data.asset_id)
				&& !FolderManager.isPublicOrphan(data.asset_id)
				&& !isMarkedForDeletion && !isChildrenRestricted;
	};
	...
	FolderManager.folderSupportsUploading = function nodeSupportsUploading(data) {
		...
		return !(FolderManager.disableUploadForNode(nodeID))
				&& canContainAssets && canEditFolder && !isMarkedForDeletion;
	}
	...
	FolderManager.getFolderData = function getFolderData(nodeID, callback,
			doRequest) {
		if (folders[nodeID]) {
			var clone = {};
	...
	FolderManager.clearCachedURLBreadcrumb = function getCachedURLBreadcrumb(
			ids) {
		var accessor = ids.join("/");
		delete breadcrumbCache[accessor];
	}

	FolderManager.handleURLBreadcrumb = function handleURLBreadcrumb(ids,
			lastName) {
	...
	FolderManager.getFolderBreadcrumb = function getFolderBreadcrumb(data, ids) {
		...
		if (data.path_list) {
			// If we don't have a list of IDs for this path, or there's only one
			// path, then just choose the first one and be done with it.
			...
			if (nodeID === "ARTESIA.PUBLIC.TREEN"
					|| nodeID === "ARTESIA.PUBLIC.TREEY"
					|| nodeID === (userID + "N") || nodeID === (userID + "Y"))
				path = {
					'tree_descriptor' : {
						'tree_id' : nodeID
					},
		...
		if (path) {
			...
			if (path.parents && path.parents.length) {
				...
				if ((FolderManager.isPublicFolders(lastFolderId) || FolderManager
						.isMyFolders(lastFolderId))
						|| (!(FolderManager.isPublicFolders(lastFolderId) || FolderManager
								.isMyFolders(lastFolderId)) && !(FolderManager
								.isPublicFolders(firstFolderId) || FolderManager
								.isMyFolders(firstFolderId))))
					path.parents.reverse();
			}

			if ((path.parents && !path.parents.length)
					|| (path.parents.length && !path.parents[0].children)) {
				var treeID = path.tree_descriptor.tree_id;
				var treeName = undefined;

				if (treeID === "ARTESIA.PUBLIC.TREE"
						|| treeID === "ARTESIA.PUBLIC.TREEY") {
	...
	FolderManager.updateFolderDeleteStatus = function(foldersList, deleted) {
		if (foldersList && foldersList.length) {
			...
			if (sidebar) {
				var contentArea = sidebar.getChildView(FolderView)
						.contentArea();
				var node = undefined;
